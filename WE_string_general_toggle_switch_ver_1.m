function WE_string_general_toggle_switch_ver_1(number_of_string_nodes,taus,kappa,nreps,string_iter,Tmove,Tavg,timestep,endtime,save_location,parameter_set,radius,cores)

% WE_string_general_toggle_switch_ver_1(number_of_string_nodes,taus,kappa,nreps,string_iter,Tmove,Tavg,timestep,endtime,save_location,gts_param,radius,cores)
% This function performs the WE-string method on a general toggle switch
...
% parameter_set = determines which parameter set (I,II,III) of the general
% switch is used
...
total_number_species = 6; %gives the number of species in the system.
...
interpolation_of_genes = (string_a_protein - initial_a_1)*(gene_on - gene_off)/(initial_a_2 - initial_a_1)+gene_off;%y-coord of prerelaxation string

string_nodes_forwards = flipud(horzcat(string_a_protein,string_b_protein,interpolation_of_genes,flipud(interpolation_of_genes),flipud(interpolation_of_genes),interpolation_of_genes));
string_nodes_backwards = flipud(string_nodes_forwards);

%***********************************************************************
model_name = ['general_switch_' parameter_set '.net']; %name and location of the BioNetGen file. In local directory

bngsimroot = '~/BioNetGen-2.2.2-stable/bin/run_network_x86_64-linux'; %BionetGen simulation root

%%************************************************************
basinA = [80,0,1,0,0,1]; %Center of Basin A
basinB = [0,80,0,1,1,0]; %Center of Basin B
%******************************************************************
...
output_folder = ['trajectories/initial_conditions/general_switch_' parameter_set '_init/'];
...
for i = 1:number_of_string_nodes
    ...
    output = [output_folder 'general_switch_' parameter_set '_' num2str(i) '.net'];
    
    overwrite_file = regexp(fileread(model_name),'\n','split');
    
    
    a = string_nodes_forwards(i,1);
    b = string_nodes_forwards(i,2);
    Aon = string_nodes_forwards(i,3);
    Bon = string_nodes_forwards(i,4);
    Aoff = string_nodes_forwards(i,5);
    Boff = string_nodes_forwards(i,6);
    
    overwrite_file{27} = sprintf('    1 a()      %d',a);
    overwrite_file{28} = sprintf('    2 b()      %d',b);
    overwrite_file{29} = sprintf('    3 Aon()    %d',Aon);
    overwrite_file{30} = sprintf('    4 Bon()    %d',Bon);
    overwrite_file{31} = sprintf('    5 Aoff()   %d',Aoff);
    overwrite_file{32} = sprintf('    6 Boff()   %d',Boff);
